...
- `src/agent_scaffold/{__init__,cli,_template}.py` — the tool itself (the
  CLI you're editing).
- `src/agent_scaffold/source-structure/common/` — the **template payload**:
  `CLAUDE.md` and the `.claude/{skills,commands,rules,agents,hooks}/`
  category dirs, with their actual content. These are the files
  `agent-scaffold` copies into a target project as-is. Editing them changes
  what end users receive, not how this repo behaves. Each `.claude/`
  ...
  gets copied, so a target project's directories only contain the real
