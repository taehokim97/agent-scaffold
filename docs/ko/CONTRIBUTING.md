...
스킬·커맨드·룰·에이전트·훅을 영문과 함께 국문으로도 작성한다면,
`source-structure/` 아래의 영문 버전이 CLI가 실제로 배포하는 버전입니다 —
이 저장소로 되돌아가는 링크 없이 독립적으로 작성하세요(다른 사람 프로젝트로
복사되면 그런 링크는 깨집니다). 국문 번역은 `docs/ko/<카테고리>/<이름>/...`
경로에 미러링해서 두세요. 이 파일은 저장소를 읽는 사람을 위한 것으로, 대상
프로젝트에는 복사되지 않습니다. 번역본 맨 위에는 실제로 배포되는 영문
원본으로 돌아가는 링크를 한 줄 추가하세요.
